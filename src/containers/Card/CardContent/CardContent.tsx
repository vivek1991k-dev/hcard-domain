...
export const CardContentBase = (props: ICardContent) => {
    ...
    const generateMultiFieldRowData = (rowData: ICardFieldData[], cols: number) => {
        return rowData.map(fieldData => {
            const fieldValue = getFieldValue(fieldData);
            return (
                <Col key={fieldData.label} span={cols}>
                    <CardValue addDivider={false} label={fieldData.label} value={fieldValue} />
                </Col>
            );
        })
    }
            ...
            return rowData.map(fieldData => {
                ...
                return (
                    <Row key={fieldData.label}>
                        <Col span={24}>
                            <CardValue addDivider={true} label={fieldData.label} value={fieldValue} />
                        </Col>
                    </Row>
                );
            });
    ...
    const generateContent = (): React.ReactNode[] => cardMetadata.content.map(rowData => {
